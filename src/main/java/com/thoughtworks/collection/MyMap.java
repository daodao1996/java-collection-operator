...
public class MyMap {
    ...
    public String getLetters(int inputNumber){
        int firstLetterIndex;
        int secondLetterIndex;
        if(inputNumber%26==0){
            firstLetterIndex = inputNumber/26 - 1;
            secondLetterIndex = 26;
        }else{
            firstLetterIndex = (int)Math.floor(inputNumber/26);
            secondLetterIndex = inputNumber % 26 == 0?26:inputNumber%26;
        }

        return letterList.get(firstLetterIndex-1) + letterList.get(secondLetterIndex-1);
    }
}
